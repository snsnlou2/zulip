...
def main() -> None:
    args = ['--follow-imports', 'silent', '--ignore-missing-imports', '--check-untyped-defs', '--sqlite-cache', '--show-error-codes']

    src_files = get_src_file_list('.')
    output, _ = run_mypy(src_files, [], [], args)
    file = open('mypy_test_report.txt', 'w')
    file.write(output)
    file.close()
